    ...
    <div class="space-y-2 lg:space-y-0 lg:space-x-4 mt-4">
        ...
        <div class="relative lg:inline-flex bg-gray-100 rounded-xl">
           <x-posts.dropdown-menu>
                ...
                @foreach($categories as $category)
                    <x-posts.dropdown-item href="/categories/{{$category->slug}}" :active="request()->is('categories/' .$category->slug)">
        ...
        <div class="relative flex lg:inline-flex items-center bg-gray-100 rounded-xl px-3 py-2">
            <form method="GET" action="#">
                <input type="text" name="search" placeholder="Search" value="{{request('search')}}" class="bg-transparent placeholder-black font-semibold text-sm">
            </form>
        </div>
    </div>
</header>